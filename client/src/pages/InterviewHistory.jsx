...
const InterviewHistory = () => {
    ...
    useEffect(() => {
        const getMyInterviews = async () => {
            try {
                console.log("🚀 Frontend: API call kar raha hoon...");
                ...
                console.log("🔢 Kitne interviews aaye:", Array.isArray(result.data) ? result.data.length : "Not an array");
                // Agar array nahi aa raha toh force array bana do
        ...
        <div className='w-[90vw] lg:w-[70vw] max-w-[90%] mx-auto'>
            ...
            <div className='grid gap-6'>
                {interviews.map((item, index) => (
                    <div key={index} onClick={()=>navigate(`/report/${item._id}`)} className='bg-white p-6 rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 cursor-pointer border border-e-gray-100'>
                        <div className='flex flex-col md:flex-row md:items-center md:justify-between gap-4'>
                            ...
                            <div className='flex items-center gap-6'>
                                ...
                                <span className={`px-4 py-1 rounded-full text-xs font-medium ${item.status === "Completed"
                                    ? "bg-emerald-100 text-emerald-100" : "bg-yellow-100 text-yellow-700"}`}></span>
                            </div>
                        </div>
                    </div>))}
            </div>}

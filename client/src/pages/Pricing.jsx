...
const Pricing = () => {
  ...
  const plans = [
    {
      ...
      price: '₹0',
      credits: 100,
      description: "Perfect for beginners starting interview preperation.",
      features: [
    ...
    },
    {
      ...
      price: '₹500',
      credits: 650,
      description: "Bestvalue for serious job preperation.",
      features: [
  ...
  return (
    <div className='min-h-screen bg-linear-to-br from-gray-50 to-yellow-50 py-16 px-6'>
      <div className='max-w-6xl mx-auto mb-14 flex items-start gap-4'>
        ...
        <div className='text-center w-full'>
          ...
          <p className='text-gray-500 mt-3 text-lg'>Flexible pricing to match your interview preperation goals.</p>
        </div>
      </div>
      <div className='grid md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto'>{plans.map((plan) => {
        ...
        return (
          ...
          >
            ...
            <div className='mt-4'>
              ...
              <p className='text-gray-50 mt-1'>{plan.credits}Credits</p>
            </div>
            {/* Description */}
            <p className='text-gray-500 mt-4 text-sm leading-relaxed'>{plan.description}</p>
            {/* Features */}
            <div className='mt-6 space-y-3 text-left'>{plan.features.map((features, index) => (
              <div key={index} className='flex items-center gap-3'>
                <FaCheckCircle className='text-emerald-500 text-sm'/>
                <span className='text-gray-700 text-sm'>{features}</span>
              </div>
            ))}</div>
            {!plan.default && <button className={`w-full mt-8 py-3 rounded-xl font-semibold transition ${isSelected ? "bg-emerald-600 text-white hover:opacity-90" : "bg-gray-100 text-gray-700 hover:bg-emerald-50"}`}>{isSelected ? "Proceed to Pay" : "Select Plan"}</button>}
          </motion.div>
        )
      })}</div>  
    </div>

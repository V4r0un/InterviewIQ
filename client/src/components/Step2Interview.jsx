...
const Step2Interview = ({interviewData, onFinish}) => {
  ...
  const videoSoucre = voiceGender === "male" ? maleVideo : femaleVideo
  ...
  const finishInterview = async () => {
    ...
    try {
        const result = await axios.post(
            ...
            { interviewId },           // ← This is correct if interviewId variable exists
            { withCredentials: true }
        );
        ...
        console.error("Error finishing interview:", error.response?.data || error.message);
        
        // Optional: Show user-friendly message
        const errorMsg = error.response?.data?.message || "Failed to finish interview";
        alert(errorMsg);
    }
  };
  ...
  return (
    <div className='min-h-screen bg-linear-to-br from-emerald-50 via-white to-teal-100 flex items-center justify-center py-4 sm:p-6'>
      <div className='w-full max-w-350 min-h-[80vh] bg-white rounded-3xl shadow-2xl border-gray-200 flex flex-col lg:flex-row overflow-hidden'>
        ...
        <div className='w-full lg:w-[35%] bg-white flex flex-col items-center p-6 space-y-6 border-r border-gray-200'>
          ...
          <div className='w-full max-w-md bg-white border border-gray-200 rounded-2xl shadow-md p-6 space-y-5'>
            <div className='flex justify-between items-center'>
              <span className='text-sm text-gray-50'>Interview Status</span>
        ...
        <div className='flex-1 flex flex-col p-4 sm:p-6 md:p-8 relative'>
          ...
          {!isIntroPhase && <div className='relative mb-6 bg-gray-50 p-4 sm:p-6 rounded-2xl border-gray-200 shadow-sm'>
          ...
          />
          {!feedback ? (<div className='flex justify-center gap-4 mt-6'>
            <motion.button
              onClick={toggleMic}
              whileTap={{scale:0.9}}        
            ...
            <motion.div
              initial={{opacity:0}}
              animate={{opacity:1}}
              className='mb-6 bg-emerald-50 border border-emerald-200 p-5 rounded-2xl shadow-sm'
              onClick={handleNext}>
                <p className='text-emerald-700 font-medium mb-4'>{feedback}</p>
                <button className='w-full bg-linear-to-r from-emerald-600 to-teal-500 text-white py-3 rounded-xl shadow-md hover:opacity-90 transition flex in-checked: justify-center gap-1'>Next Question <BsArrowRight size={18}/></button>
            </motion.div>
          )}
        </div>
      </div>

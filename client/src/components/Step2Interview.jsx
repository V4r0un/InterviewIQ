...
const Step2Interview = ({ interviewData, onFinish }) => {
  ...
  const startMic = () => {
    ...
    if (!recognition || isAIPlaying || isListening) return;
        ...
        <div className='flex-1 flex flex-col p-4 sm:p-6 md:p-8 relative'>
          ...
          {!feedback ? (
            ...
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className='mb-6 bg-emerald-50 border border-emerald-200 p-5 rounded-2xl shadow-sm cursor-pointer'
              onClick={handleNext}
            >
              <p className='text-emerald-700 font-medium mb-4'>{feedback}</p>
              <button className='w-full bg-linear-to-r from-emerald-600 to-teal-500 text-white py-3 rounded-xl shadow-md hover:opacity-90 transition flex items-center justify-center gap-2'>

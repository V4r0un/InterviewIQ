...
const Step1SetUp = ({onStart}) => {
  ...
  const handleStart = async () => {
    ...
    const payload = {
      ...
      mode,
      resumeText: resumeText?.trim() || "",        // ensure string
      projects: Array.isArray(projects) ? projects : [],
      skills: Array.isArray(skills) ? skills : []
    };

    console.log("Sending payload to generate-questions:", payload);   // ← Very important
    ...
    try {
      const result = await axios.post(
        ...
        { 
          ...
          headers: { 'Content-Type': 'application/json' }   // explicit is safer
        }
      );
      ...
      if (error.response) {
        ...
        console.error("Error data from server:", error.response.data);   // ← This will tell you exactly what's wrong
        alert(error.response.data?.message || error.response.data?.error || "Server rejected the request. Check console.");
      } else if (error.request) {
        console.error("No response from server");
        alert("No response from server. Check your internet or server is running?");
      } else {
              ...
              <div className='space-y-6'>
                ...
                {!analysisDone && (
                  <motion.div
                    ...
                    className='border-2 border-dashed border-gray-300 rounded-xl text-center cursor-pointer hover:border-green-500 transition'>
                ...
                {analysisDone && (
                  <motion.div
                    ...
                    className='bg-gray-50 border border-gray-200 rounded-xl p-5 space-y-4'>
                      ...
                      {projects.length > 0 && (
                        <div>
                          <p className='font-semibold text-green-700 mb-1'>Projects:</p>
                      ...
                      {skills.length > 0 && (
                        <div>
                          <p className='font-semibold text-green-700 mb-1'>Skills:</p>
